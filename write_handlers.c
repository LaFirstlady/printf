...

{
	int i, padd_start = 1;
	 ...
	 if (prec == 0 && ind == BUFF_SIZE - 2 && buffer[ind] == '0')
		 buffer[ind] = padd = '\0'; /* width is displayed with padding''*/
	 ...
	 if (width > length)
	 {
		 for (i = 1; i < width - length + 1; i++)
			 buffer[i] = padd;
		 buffer[i] = '\0';
		 if (flags & F_MINUS && padd == '\0')/** assign extra char to left of buffer*/
		 {
			 if (extra_c)
				 buffer[--ind] = extra_c;
...
int write_pointer(char buffer[], int ind, int length, int width, int flags, char padd, char extra_c, int padd_start)
{
	...
	if (width > length)
	{
		...
		else if (!(flags & F_MINUS) && padd == '0')
		{
			...
			buffer[1] = '0';
			buffer[2] = 'x';
			return (write(1, &buffer[padd_start], (padd_start) + write(1, &buffer[ind], length - (1 - (padd_start) - 2));
		}
	}
	buffer[--ind] = 'x';
	buffer[--ind] = '0';
